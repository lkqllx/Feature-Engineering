...
def plot_corr(df, corr_mat):
    ...
    plt.show()
    # plt.savefig(fname='features_corr.png', dpi=900)

def run_regr(train, test):

    regr = sm.OLS(train['Y_M_1'], sm.add_constant(train.iloc[:, 14:])).fit()
    # print(regr.summary())
    try:
        y_pred = regr.predict(sm.add_constant(test.iloc[:, 14:]))
...
if __name__ == '__main__':
    ...
    def mp_reg(epoch):
        ...
        try:
            tvalues.to_csv(f'result/Tvalues/{ticker}/Tvalue_reg_{test_data.date[0]}.csv', index=False)
        except:
            os.mkdir(f'result/Tvalues/{ticker}')
            tvalues.to_csv(f'result/Tvalues/{ticker}/linear_reg_{test_data.date[0]}.csv', index=False)
    ...
    for idx in [170]:
        ...
        record = pd.DataFrame(record[:,:2], columns=['date', 'r2'])
